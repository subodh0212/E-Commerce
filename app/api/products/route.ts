...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (category && category.toLowerCase() !== "new-in" && category.toLowerCase() !== "all") {
        whereClause.category = { equals: category.toLowerCase(), mode: "insensitive" };
      }
      if (brand) {
    ...
    let filtered = [...ATELIER_MOCK_PRODUCTS];
